...
Bowtie2 |	align to reference	| SKIP | unaligned reads as fastq|./trimmomatic_paired/ and ./trimmomatic_unpaired | 	
MetaSpades | Assemble reads | | | |
Quast | check assembly quality |(on vivi's jupyter) | | |
CheckM | host contamination | | | |	| | | |
phyloflash pipeline | taxonomic composition | (vivi used MetaPhlAn| | |	
...
After the trimmomatic removed the adapters, let's run the quality analysis again on the paired reads to see if the quality improved
...
#### 2.2 Grouping trimmed reads by 

Now I need to unzip all of the samples and group them the host they were sampled
on (ex: dandelion, apple, trefoil). This way, we can filter out their host genome with bowtie2 using the corresponding reference genome
...
## This is based on metadata in Floral_metagenomics/data/metadata_sequencing.xlsx
...
We're gonna run the trimmomatic paired and unpaired trimmed reads through Bowtie2 to align the reads to their corresponding host genome. The samples were washed off of flower petals, (info in `Floral_metagenomics/data/metadata_sequencing.xlsx`) so for each sample I will be aligning to corresponding genomes from spreadsheet:
/local/workdir/Daphne/Floral_metagenomics/data/metadata_host_flower_genomes.xlsx

First, I need to create reference indices for every host genome that I'll be using. I found the RefSeq's online through NCBI genome finder, downloaded the FTP `_genomic.fna.gz` file to my computer, then transfered it to the server.
...
Here is a test bowtie of the first samples: 93. This will tell us approx how long each one takes
```{bash}

cd /local/workdir/Daphne/Floral_metagenomics/data/

## name index
index="Chichorium_indices/Chichorium_index"

for i in *R1.fastq.gz; do 
  output_dir="${i%_R1.fastq.gz}_bowtie2"
  bowtie2 ---sensitive-local -p 32 --seed 4 -x $index -1 $i -2 ${i/%R1.fastq.gz/R2.fastq.gz} -S SAMPLE_mapped_and_unmapped.sam
done 
...
#### Modify these variables for each group of host reference alignments
index="Chichorium_indices/Chichorium_index"
