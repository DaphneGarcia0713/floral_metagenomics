...
for sample in ./*; do
  for R1 in ${sample}/SAMPLE_host_removed_R1.fastq; do
    ...
    kaiju -t "${kaiju_path}/nodes.dmp" -f "${kaiju_path}/kaiju_db_nr_euk.fmi" -i $R1 -j $R2 -o ../../09_kaiju/1_raw_kaiju_euk/${output_name}_kaiju.txt -z 24 -v
    echo $(date)
  done
done
  #took 11 hours
...
I combine the reference proteins with cat x y z > all.fasta

NOTE: the reference proteins that I used are described in
`data_3/14_blast_orthologs/1_references_and_outgroups/pectin_lyase_refs/README.md` NO LONGER THERE
...
num=""
ortholog="pectin_lyase"
for sample in 04_metaspades/${num}*; do   
    # Input
    query_nucleotides="14_blast_orthologs/1_references_and_outgroups/${ortholog}_references/all_references.fasta"
  
    # Output directories
    OUTDIR="14_blast_orthologs/2_ortholog_search/$(basename "$sample")/${ortholog}_TWO"
...
for subdir in 14_blast_orthologs/2_ortholog_search/*; do
  echo "$(basename "${subdir}")" >> 14_blast_orthologs/2_ortholog_search/all_${ortholog}_matches.txt
  cat ${subdir}/${ortholog}/blast_results.filtered.tsv | wc -l >> 14_blast_orthologs/2_ortholog_search/all_${ortholog}_matches.txt
done
  # took 1 min
```
  outputs are in pectin_lyase or polygalacturonase subdirectories within `data_3/14_blast_orthologs/2_ortholog_search` that contain blast_results.filtered.tsv, which are a list of references and their matching nodes according to the BLAST search. Also outputted is `data_3/14_blast_orthologs/2_ortholog_search/all_{pectin_lyase/polygalacturonase}_matches.txt`, which lists the number of matches found per sample
...
for sample in 14_blast_orthologs/2_ortholog_search/${num}* ; do
    name=$(basename "$sample")
    tsv="${sample}/${ortholog}/blast_results.filtered.tsv"
    ...
    for node in $nodes; do
        ...
        ' "$fasta" >> 14_blast_orthologs/3_orthologs_from_all_samples/${ortholog}.fasta
    done
done
  #took <5sec
```
Note: I removed >4_FN176_NODE_2 from pectin_lyase.fasta because it was incredibly long (200k bp) and did not BLAST to acineto with ncbi


#### 14.4 aligning with MAFFT
Before this step, I combined ortholog's reference and outgroup fastas from `14_blast_orthologs/1_references_and_outgroups` together with our samples from `14_blast_orthologs/3_3_orthologs_from_all_samples` with cat, and output it in `14_blast_orthologs/3_orthologs_from_all_samples`
...
14_blast_orthologs/3_orthologs_from_all_samples/${ortholog}_PLUS_references_and_outgroups.fasta > \
14_blast_orthologs/4_aligned_orthologs_MAFFT/${ortholog}.aln
  # took ~20 sec
  
# --auto: chooses best strat for this data, which is FFT-NS-2
...
ortholog="pectin_lyase"
/programs/iqtree-2.2.2.6-Linux/bin/iqtree2 -s 14_blast_orthologs/4_aligned_orthologs_MAFFT/${ortholog}.aln -m MFP \
--prefix 14_blast_orthologs/5_IQ_tree/${ortholog} -B 1000
