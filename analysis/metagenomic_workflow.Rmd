...
for sample in 06_sample_contigs/*.fasta; do 
  ...
  echo $depth
  singularity run --bind $PWD --pwd $PWD /programs/metabat-2.16/metabat.sif metabat2 -i $sample -a 06_depth_files/${depth}_depth.txt -o 06_MetaBat2/just_38_test/${depth}_bin --seed 42 -m 1500 -s 100000 --maxEdge 250
done
...
docker1 run --rm \
  ...
  -v /local/workdir/Daphne/Floral_metagenomics/data/06_MetaBat2/just_38_test:/input \
  -v /local/workdir/Daphne/Floral_metagenomics/data/07_CheckM/just_38_test:/output \
...
qr_m1500_s100k_maxE250 <- read.table(file="data/07_CheckM/07_m1500_s100k_maxE250_checkM2_output/quality_report.tsv", sep="\t")

    # change the name (before <-) and subdirectory in 07_CheckM
```
Note: I used this to look at the quality of just_38_test, which is my MetaBat and CheckM run of a single sample, paired_38_Chichorium. This outputted one bin, and the CheckM was identical to it's 07_m1500_s100k_maxE250 results, which tells me that MetaBat and CheckM really do only contain 1 bin for most samples. 



### 7 GTDB-k bin taxonomy 
...
```{r Visualize_CheckM_R, eval = FALSE}
getwd() # should be Floral_metagenomics
gtdbtk_summary <- read.table( file="data/07_gtdbtk_out/gtdbtk.bac120.summary.tsv", sep="\t")
gtdbtk_summary
getwd()
    # change the name (before <-) and subdirectory in 07_CheckM
```
...
kraken2-build --download-library fungi --db  /local/workdir/Daphne/Floral_metagenomics/databases --threads 64
